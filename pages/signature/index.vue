<template>
    <div class="demandes_validation_page">
        ...
        <div class="d-flex justify-content-between align-items-center mb-4">
            <h1>LISTE DES DEMANDES VALIDÉES</h1>
        ...
        <!-- Tableau des demandes validées -->
        <div class="table_block_list">
            <Table
                :columns="columns"
                :rows="filtered_demandes"
                :type_but_link="true" 
                but_link_path="signature/" 
...
// Variables réactives
const liste_demandes_validees = ref([]) // Liste originale
...
/* METHODS */
const getDemandesValidees = async () => {
    loading.value = true;

    try {
        // 1️⃣ Récupérer les demandes + nom du user + items niv_val = VALIDE
        ...
        const result = data.map(row => ({
            ...row,
            nbrnv: row.ses_demItems?.length ?? 0,   // nombre d'items validés
        ...
        liste_demandes_validees.value = result;
        filtered_demandes.value = [...result];

        console.log('liste demandes validées', result);
...
const filterData = () => {
    if (!search_term.value.trim()) {
        filtered_demandes.value = [...liste_demandes_validees.value];
        return;
    }
    loading.value = true;
    const term = search_term.value.toLowerCase().trim();
    
    filtered_demandes.value = liste_demandes_validees.value.filter(item => {
